...
class TemplateContentScript extends cozy_clisk_dist_contentscript__WEBPACK_IMPORTED_MODULE_0__.ContentScript {
  async ensureAuthenticated({ account }) {
    ...
    if (credentials) {
      try {
        ...
        this.log('warn', 'Autologin failed: ', err.message)
        await this.showLoginFormAndWaitForAuthentication()
        return true
      }
    }
  ...
  async ensureNotAuthenticated() {
    ...
    const authenticated = await this.runInWorker('checkAuthenticated')
    if (!authenticated) {
      this.log('info', '🤖 already no Authenticated')
      return true
    }

    await this.logout()
    this.log('info', '🤖 after logout')
    return true
  }
  ...
  async fetch(context) {
    this.log('info', '🤖 fetch')
    if (this.store) {
      const { login, password } = this.store
      if (login && password) {
  ...
  async checkAuthenticated() {
    if (!window.checkAuthenticatedEventSubscribed) {
      ...
      window.checkAuthenticatedEventSubscribed = true
    }
  ...
  async navigateToLoginForm() {
    ...
    this.log('info', '🤖 end of navigateToLoginForm')
  }

  async autologin(credentials) {
    this.log('info', 'Autologin', credentials)
    await this.clickAndWait(loginLinkSelector, '#username')
    await this.runInWorker('fillText', '#username', credentials.login)
    await this.runInWorker('fillText', '#password', credentials.password)
    await this.runInWorker('click', '[type=submit]')
